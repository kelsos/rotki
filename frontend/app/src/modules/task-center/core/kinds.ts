...
/**
 * The single source of truth for kind ordering and group titles. Array order is the
 * display + selection priority (highest first): it drives group ordering and which
 * running activity the header bar labels (`current`). Adding a kind is one entry here
 * (plus the enum) — there is no separate priority list to keep in sync.
 *
 * Kinds that never produce an activity (deferred: db-upgrade/data-migration; unused:
 * online-events) are intentionally omitted; they rank last via {@link kindRank} and
 * never appear.
 */
